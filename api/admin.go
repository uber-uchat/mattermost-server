...
func adminUpdateUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-c.App.Srv.Store.User().AdminUpdate(user, false); result.Err != nil {
		...
		if rusers[0].Email != rusers[1].Email {
			c.App.Go(func() {
				if err := c.App.SendEmailChangeEmail(rusers[1].Email, rusers[0].Email, rusers[0].Locale, c.GetSiteURLHeader()); err != nil {
					l4g.Error(err.Error())
				}
			})
		...
		if rusers[0].Username != rusers[1].Username {
			c.App.Go(func() {
				if err := c.App.SendChangeUsernameEmail(rusers[1].Username, rusers[0].Username, rusers[0].Email, rusers[0].Locale, c.GetSiteURLHeader()); err != nil {
					l4g.Error(err.Error())
				}
			})
		}
