...
func (a *App) UpdateOooRequestUser(userId string, user *model.User, delayedUpdate bool) *model.AppError {
	...
	if user.NotifyProps[model.AUTO_RESPONDER_FROM_DATE] == "" {
		fromDate = time.Now()
	} else {
		fromDate, _ = time.Parse("2006-1-2", user.NotifyProps[model.AUTO_RESPONDER_FROM_DATE])
	}

	fromTime := user.NotifyProps[model.AUTO_RESPONDER_FROM_TIME]
	toTime := user.NotifyProps[model.AUTO_RESPONDER_TO_TIME]

	var toDate time.Time
	if user.NotifyProps[model.AUTO_RESPONDER_TO_DATE] == "" {
		toDate = fromDate.AddDate(200, 0, 0)
	} else {
		toDate, _ = time.Parse("2006-1-2", user.NotifyProps[model.AUTO_RESPONDER_TO_DATE])
	}
