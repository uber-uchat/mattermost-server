...
func (a *App) UpdateOooRequestUser(userId string, user *model.User, delayedUpdate bool) *model.AppError {
	...
	if user.NotifyProps[model.FromDate] == "" {
		fromDate = time.Now()
	} else {
		fromDate, _ = time.Parse("2006-1-2", user.NotifyProps[model.FromDate])
	}

	fromTime := user.NotifyProps[model.FromTime]
	toTime := user.NotifyProps[model.ToTime]

	var toDate time.Time
	if user.NotifyProps[model.ToDate] == "" {
		toDate = fromDate.AddDate(200, 0, 0)
	} else {
		toDate, _ = time.Parse("2006-1-2", user.NotifyProps[model.ToDate])
	}
